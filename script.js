...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (savedKey) {
    ...
    keyStatus.className = "text-sm text-emerald-400 mt-3";
  } else {
    keyStatus.textContent = "No API key saved yet.";
    keyStatus.className = "text-sm text-slate-400 mt-3";
  }
});
...
saveKeyBtn.addEventListener("click", () => {
  ...
  if (!key) {
    ...
    keyStatus.className = "text-sm text-red-400 mt-3";
    return;
  }
...
clearBtn.addEventListener("click", () => {
  editor.setValue("");
  resultCards.innerHTML = `<div class="text-slate-400">Analysis results will appear here.</div>`;
...
analyzeBtn.addEventListener("click", async () => {
  ...
  resultCards.innerHTML = `<div class="text-slate-300">Analyzing your code...</div>`;
